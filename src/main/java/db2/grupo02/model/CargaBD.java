...
public class CargaBD {
	...
	public static List<Venta> generarVentas(Sucursal sucursal, Cliente[] clientes, String formaDePago, 
		...
		for (int i = 0; i < cantidadDeVentas; i++) {
			...
			//ventas.add(new Venta(nroDeTicket, fecha, atencion, cobro, sucursal, cliente, items));
		}
		return ventas;
	}
	...
	public static void main(String[] args) {
		...
		Cliente c2 = new Cliente(42000002, "apellido", "Chino",
				new Domicilio("Maipu ", 128, "CABA", "Buenos Aires"), medicus);
		...
		*/
		...
		Sucursal s1 = new Sucursal(1, "Sucursl CABA",
				new Domicilio("Av de Mayo", 510, "CABA", "Buenos Aires"), Arrays.asList(e1, e2));
		
		Sucursal s2 = new Sucursal(2, "Sucursal Lanus", 
				new Domicilio("Mitre", 200, "Lanus", "Buenos Aires"),Arrays.asList(e1, e2));
		
		Sucursal s3 = new Sucursal(3, "Sucursal Once", 
				new Domicilio("Av. Puerreydon", 750, "CABA", "CABA"),Arrays.asList(e1));
		
		
		// SE DEFINES LAS VENTAS PARA QUE SEAN ENTRE ENERO DESDE EL 2021 HASTA LA FECHA ACTUAL
		
		List<Venta> ventas = new ArrayList<Venta>();
		
		// SE REALIZA 30 VENTAS ALEATORIAS PARA LA SUCURSAL DE LANUS
		ventas.addAll(generarVentas(s1, new Cliente[] { c1, c2}, "efectivo", productos,LocalDate.of(2021, 01, 01), LocalDate.now(), 30));
		// SE REALIZA 25 VENTAS ALEATORIAS PARA LA SUCURSAL DE LANUS
		ventas.addAll(generarVentas(s2, new Cliente[] { c1, c2}, "debito", productos,LocalDate.of(2021, 01, 01), LocalDate.now(), 25));
		// SE REALIZA 35 VENTAS ALEATORIAS  PARA LA SUCURSAL DE ONCE
		ventas.addAll(generarVentas(s3, new Cliente[] { c1, c2}, "efectivo", productos,LocalDate.of(2021, 01, 01), LocalDate.now(), 35));
